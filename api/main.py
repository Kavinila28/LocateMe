"""
LocateMe — FastAPI REST Backend Service
...
app = FastAPI(
    ...
    description="RESTful AI-assisted missing person screening service for public feeds.",
    version="1.0.0",
)
...
def get_pipeline():
    ...
    if _gallery is None:
        _gallery = GalleryManager(gallery_dir=REG_DIR, detector=_detector, generator=_generator)
...
def health_check():
    """Returns server status, registered gallery count, and compute device."""
...
def list_gallery():
    ...
    for p in gallery.persons:
        img_filename = Path(p.image_path).name
        persons_summary.append(
            PersonSummary(
                person_id=p.person_id,
                name=p.name,
                image_url=f"/media/registered/{img_filename}",
...
async def register_person(
    ...
    Precomputes and caches 512-D face embeddings.
    """
    detector, generator, gallery = get_pipeline()
    ...
    summary = PersonSummary(
        ...
        name=reg.name,
        image_url=f"/media/registered/{save_filename}",
    ...
    return PersonRegistrationResponse(
        success=True,
        message=f"Successfully registered '{name}' in LocateMe gallery.",
        person=summary,
    )
...
def delete_person(person_id: str):
    """Remove a registered person from the gallery."""
    _, _, gallery = get_pipeline()
    target = [p for p in gallery.persons if p.person_id == person_id]
    ...
    person = target[0]
    gallery.persons = [p for p in gallery.persons if p.person_id != person_id]
    gallery._rebuild_matrix()
    gallery.save_cache()

    # Remove image file if exists
    img_p = Path(person.image_path)
    if img_p.exists() and img_p.parent == REG_DIR:
        try:
            img_p.unlink()
        except Exception:
            pass

    return {"success": True, "message": f"Person '{person.name}' ({person_id}) removed from gallery."}
    ...
    for idx, det in enumerate(face_results):
        ...
        for person, match_res in all_matches:
            cand = CandidateMatch(
                ...
                reference_image_url=f"/media/registered/{Path(person.image_path).name}",
            )
